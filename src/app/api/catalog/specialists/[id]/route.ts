...
export async function GET(
    ...
    try {
        ...
        const specialistDoc = await adminDb.collection("specialists").doc(id).get();
        ...
        const specialist = toSpecialistDTO(specialistDoc.id, specialistDoc.data()!);
        const serviceLabels = Object.fromEntries(
            specialist.services.map((service) => [service, getServiceLabel(service)])
        );
